...
KAST は Kaspa の進化の2層で構築される:

- **Covenant++ (L1)**: UTXO トークンのライフサイクル — 発行、匿名化、投票、集約、タイムロック解除。コンセンサスで強制。
- **vProgs (L2)**: 暗号化投票、Benaloh チャレンジ検証、準同型集計、選挙区横断合成。CairoVM でオフチェーン実行、ZK proof で L1 検証。

両層が完全なシステムに必要。Covenant++ がトークンインフラを、vProgs が暗号的プライバシーと投票者検証レイヤーを提供する。

---

## 概要

KAST は、Kaspa の Covenant++ (L1) と vProgs (L2) を活用した、物理・デジタルハイブリッド型の電子投票プロトコルである。
...
[投票日 — 投票所にて、1人あたり約3分]
  ...
  Phase 3: QR2 用紙に候補者名自書 → PCでスキャン・投票      (KASTVote)
           → 暗号化 Vote TX → 任意でスマホ Benaloh 検証
           QR2 用紙を投票箱に投函
...
選挙管理委員会:
  ...
  ② 各有権者に QR1 を生成・配布
     ...
     秘密鍵を QR コード化して配布
  ③ 有権者名簿から Merkle tree を構築 (ZK 検証用)
...
| DaaScore 窓 | OpTxInputDaaScore が利用可能になれば、Covenant で強制的なフェーズ間時間分離を追加可能 |
...
| リンク切断 | 物理的: QR1→QR2 の対応は投票所だけが知る。ZK: オンチェーンリンクも暗号的に切断 |
| ブース隔離 | 投票ブースが画面撮影を物理的に防止。端末は外部通信を遮断 |
| 買収の無意味化 | QR1 を買っても投票先を制御不能 — QR2 は投票所で新しい鍵から生成されるため |

### 5. 投票所端末の改竄

**脅威**: 改竄された端末が投票者の選択と異なる候補者に投票する、または QR1→QR2 の対応を漏洩する。（[ベクトル10](#10-端末信頼性投票の完全性) も参照: 物理・デジタル対策）
...
| 対策 | 実装方法 |
|---|---|
| 均一な TX 構造 | 全投票 TX の value (TOKEN_VAL)、スクリプトサイズ (KASTVote = 全投票者共通549B)、構造が同一 |
...
**脅威**: KASTMint は TOKEN_VAL と covenant chain の連続性を強制するが、出力の `lockingBytecode`（送信先スクリプト）は**検証しない**。選管が匿名化ステップを飛ばして KASTVote や KASTTally アドレスに直接トークンを発行可能 — 投票者なしの「幽霊票」を生成できる。
...
### 18. Benaloh チャレンジの選択的正直性

**脅威**: 改竄された端末がチャレンジ時のみ正直に動作し、チャレンジされない場合に票をすり替える。標準の Benaloh チャレンジは投票者の自発的なチャレンジに依存するが、実際のチャレンジ率は極めて低い可能性がある（スマホ操作の壁、高齢者）。端末がプロファイリングにより、チャレンジしなさそうな投票者を選択的に狙うことも可能。

**注記**: このベクトルは KAST の並走紙投票により大部分が緩和される（紙は統計ではなく確定的な選挙後全数検出を提供）。以下はリアルタイム検出層を強化する提案。

**対策**:

| 対策 | 実装方法 |
|---|---|
| 並走紙投票（既存） | QR2 用紙への自書候補者名が選挙後の100%検出を提供 — 統計的ではない。Benaloh は補助的リアルタイム層 |
| システム主導ランダムチャレンジ (SIRC、提案) | 投票者主導のチャレンジをシステム駆動に置換: 端末が暗号化投票にコミット後、Kaspa ブロックハッシュ（コミット時点で予測不能）がチャレンジ有無を決定。チャレンジ率は固定・均一 — 端末は予測も選択的不正も不可能 |
| 暗号化正当性の ZK 証明 (vProgs、提案) | 端末が暗号化投票 + 暗号化正当性の ZK proof を出力。全投票を確定的に検証 — チャレンジ不要。オンチェーン proof 検証に vProgs が必要 |
...
|---|---|---|---|---|
| 日本 | 5,500 万人 | 13 時間 | ~3,525 | 35% |
| アメリカ | 1.44 億人 | 13 時間 | ~9,230 | 92% |
| EU 全体 | 1.85 億人 | 複数日 / 加盟国分散 | 分散で対応可 | — |
| インド | 6.2 億人 | 複数日に分散 | 分散で対応可 | — |

KAST では 1 投票者あたり 3 TX（Mint + 匿名化 + 投票; Mint は最大4バッチで実効TX数を削減）を必要とする。日本の国政選挙は Kaspa 処理能力の 35% 程度で処理可能。大規模選挙（米国、EU）は複数日投票やより高い BPS で対応。
