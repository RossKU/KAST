...
| セキュア消去 | TEE ベースのメモリ隔離により、マッピングが永続ストレージに到達しない。ディスクセクタは暗号的にワイプ（ファイル削除ではなく）し、フォレンジック復元を防止 |
...
| 鍵紛失の回復 | HSM バックアップを地理的に分散した鍵セレモニーで管理。両方の鍵が回復不能に紛失した場合、当該投票所の Mint が停止 — 他の投票所は独立して継続。選挙後の回収 (release/recover) は electionAuthority 鍵で実行し、commissioner 鍵とは独立 |
...
### 19. Vote UTXO マージ攻撃（多入力による票破壊）

**脅威**: KASTVote は全投票者で同一スクリプト（均一性設計）。`vote()` は `tx.outputs.length == 1` と `tx.outputs[0].value == TOKEN_VAL` を強制するが、`tx.inputs.length` は**未制約**。ベクトル17のフロントランニングで複数の匿名鍵を奪取した攻撃者が、N個の Vote UTXO を1つの TX にまとめ、N-1 票を破壊できる。
...
攻撃TX:
  ...
  結果: 2票消費 → 1票記録。0.1 KAS は手数料として消滅。
```
...
| 対策 | 実装方法 |
|---|---|
| 同一バッチ送信（既存） | 複数の匿名鍵奪取に必要なフロントランニング窓（ベクトル17前提）を排除 |
| 前提条件の困難さ | 攻撃者はまず複数の UTXO をフロントランニング（ベクトル17）する必要があり、各 UTXO は狭いバッチ窓内の観察が必要 |
| 選挙後監査 | 紙投票枚数 vs オンチェーン投票数の不一致を検出。欠損票は調査を発動 |
| 入力数制約（将来） | `vote()` に `require(tx.inputs.length == 1)` を追加すれば多入力 TX を防止。Kaspa の入力数イントロスペクション opcode が必要 |

### 20. 選択的投票妨害（Mint 拒否）

**脅威**: ベクトル2はトークンの不正**水増し**をカバーする。逆方向も可能: 選管が特定の投票者（政治的敵対者、少数派、特定地区）への Mint を意図的に**拒否または遅延**する。JIT モデルでは投票者の QR1 は空ウォレット — Mint されなければ投票は不可能。

**対策**:

| 対策 | 実装方法 |
|---|---|
| リアルタイム Mint 監視 | 公開ダッシュボードが投票所ごとの時間帯別 Mint 数を追跡。異常な低下（投票所は稼働中だが Mint なし）で即時アラート |
| 投票者苦情チャネル | 現場の苦情受付: 閾値時間を超えてもトークンを受け取れない投票者が正式に報告。タイムスタンプ付きで投票所に紐づけ |
| 他投票所での Mint | 当該投票所が拒否した場合、近隣投票所に誘導。投票所ごとの鍵隔離（ベクトル12）により、1箇所の拒否が他を阻害しない |
| 2名の commissioner による相互牽制 | Mint には両名の署名が必要。妨害には両名の共謀が必要。独立した commissioner 任命で共謀リスクを低減 |
| 法的枠組み | 意図的な Mint 拒否はほとんどの法域で選挙妨害罪に該当。HSM 監査ログが Mint 操作（または不作為）の暗号的証拠を提供 |

### 21. セットアップ不正（コンストラクタ引数の改竄）

**脅威**: 全 KAST コントラクトはデプロイ時にパラメータ化される。コンストラクタ引数が改竄または誤設定された場合、選挙全体が侵害される — 選挙後監査まで検出できない可能性がある。

```
具体的リスク:
  a) 候補者鍵のすり替え: cand3 を選管の鍵に差し替え
     → 候補者3への票が無断でリダイレクト
  b) 候補者鍵の重複: cand2 == cand5
     → 2候補がアドレスを共有、票が合流
  c) ダミー鍵の衝突: 未使用スロットの burn 鍵が実在候補と一致
  d) electionEnd が早すぎる: 投票期間終了前に設定
     → 選挙中に recover/release が発動可能（早期トークン回収）
  e) electionEnd が遅すぎる: 預託が選挙後長期間ロック
```
...
| 対策 | 実装方法 |
|---|---|
| 公開パラメータ台帳 | 全コンストラクタ引数を選挙前に公開。候補者鍵を公式候補者名簿とクロスチェック。誰でも検証可能 |
| パラメータ検証ツール | 自動デプロイ前チェック: 鍵の重複なし、burn 鍵の衝突なし、electionEnd が有効範囲内、TOKEN_VAL が仕様と一致 |
| 多者参加のパラメータセレモニー | コンストラクタ引数を複数の独立当事者（選管 + 監視員 + 候補者）が共同で組み立て。全当事者が最終パラメータセットに署名 |
| オンチェーンパラメータコミットメント | 全コンストラクタ引数のハッシュをデプロイ前にオンチェーンにコミット。デプロイ後にバイトコードをコミットメントと照合 |
| electionEnd 安全マージン | electionEnd を投票期間終了 + バッファ（例: +24時間）に設定。早すぎる値は検証ツールが拒否 |

### 22. ビルド・デプロイチェーン攻撃（サプライチェーン）

**脅威**: ソースコードが監査済みで正しくても、デプロイされたバイトコードがソースと異なる可能性がある。攻撃経路: コンパイラバックドア (Thompson 攻撃)、ビルド環境の侵害、デプロイ時のバイトコード差替え。

```
ソース (.sil) → [コンパイラ] → バイトコード (JSON) → [デプロイ] → オンチェーン
     ✓ 監査済     ??? 未信頼      ??? 未検証          ??? 未確認
```
...
| 対策 | 実装方法 |
|---|---|
| 決定論的ビルド | コンパイラを再現可能な環境 (Docker, ピン留めイメージハッシュ) で実行。複数の独立当事者が同一ソースからコンパイルし、バイトコードハッシュの一致を検証 |
| バイトコードハッシュ台帳 | 全コンパイル済みコントラクトの SHA-256 ハッシュを選挙前に公開。デプロイ後検証: オンチェーンバイトコードハッシュ vs 台帳 |
| コンパイラピン留め | `pragma silverscript 0.1.0`（`^0.1.0` ではなく完全固定）。コンパイラバイナリのハッシュをコントラクトハッシュと共に公開 |
| 多者コンパイル | 最低3つの独立当事者（選管、野党、市民団体）がコントラクトをコンパイル。全バイトコードハッシュが一致した場合のみデプロイ |
| オンチェーンソース検証 | コントラクトソースコードをオンチェーンにコミット（OP_RETURN または別 TX）。誰でも再コンパイルして検証可能 |
...
デプロイ前および運用時の品質ゲート。(protocol) はコントラクト/チェーンで強制、(process) は組織的手順が必要。

### 選挙前

| # | ゲート | 種別 | 説明 |
|---|---|---|---|
| QC-1 | 決定論的ビルド検証 | Process | 複数の独立当事者がソースをコンパイル → バイトコードハッシュの一致を検証（ベクトル22） |
| QC-2 | コンストラクタ引数検証 | Process | 自動ツールでチェック: 候補者鍵の重複なし、electionEnd が有効範囲内、TOKEN_VAL が正しい（ベクトル21） |
| QC-3 | 多者パラメータセレモニー | Process | コンストラクタ引数を選管 + 監視員 + 候補者が共同で組み立て・署名 |
| QC-4 | オンチェーンパラメータコミットメント | Protocol | 全コンストラクタ引数のハッシュをデプロイ前にコミット。誰でも検証可能 |
| QC-5 | コンパイラバージョン固定 | Process | セマンバー範囲ではなく完全固定バージョンをロック・公開 |
| QC-6 | 鍵セレモニー | Process | Commissioner 鍵を HSM で生成、地理分散バックアップ。electionAuthority 鍵は別途生成 |
| QC-7 | 端末認証 | Process | TEE リモートアテステーションで端末ソフトウェアの完全性を選挙前に検証 |
| QC-8 | ドライラン（模擬選挙） | Process | テストネットで全工程リハーサル: Mint → Anon → Vote → Aggregate → Release → Audit |
| QC-9 | 並行選挙の分離 | Process | 同日の各選挙が独立 Master UTXO（異なる covenant_id）を使用。端末ソフトウェアが選挙 ID を検証してから処理 |

### 選挙当日

| # | ゲート | 種別 | 説明 |
|---|---|---|---|
| QC-10 | リアルタイム Mint 監視 | Process | 公開ダッシュボード: 投票所ごとの時間帯別 Mint 数。異常な低下・スパイクでアラート |
| QC-11 | バッチタイミング検証 | Process | 全投票所がランダム化バッチで送信していることを検証（個別 TX 送信でないこと） |
| QC-12 | インシデント対応手順 | Process | 文書化されたエスカレーション: 端末障害 → 予備端末; ネットワーク断 → オフラインキュー; 鍵紛失 → 隣接投票所への引継ぎ |
| QC-13 | 投票者苦情チャネル | Process | タイムスタンプ付き記録の現場受付。閾値違反で即時上位レビュー |

### 選挙後

| # | ゲート | 種別 | 説明 |
|---|---|---|---|
| QC-14 | 3層監査の実行 | Process | カウント → 署名 → 手書き検証（物理・デジタル監査セクション参照） |
| QC-15 | 紛争解決プロトコル | Process | 紙 vs チェーン不一致: (1) 紙を再集計、(2) TX 署名を検証、(3) 解決不能 → 当該投票所は紙が正、グローバルにはチェーンが正 |
| QC-16 | 結果認定 | Process | QC-14 完了後、全投票所が NORMAL ステータスの場合のみ最終結果を公示。閾値超のノイズ率の投票所は認定前に手動再集計 |
| QC-17 | 証拠保全 | Process | オンチェーンデータは永続。紙投票は管轄法の保存期間（通常1-5年）に従い封印保管。HSM 監査ログをエクスポート・アーカイブ |
| QC-18 | ポストモーテム | Process | 標準化された振り返り: インシデント記録、根本原因特定、次回選挙への改善提案 |
